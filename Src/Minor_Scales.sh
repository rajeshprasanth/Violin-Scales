...
cat > ./$workdir/${title}_Minor_Broken_Chords.ly << EOF
\version "2.19.82"
...
cat > ./$workdir/${title}_Minor_Broken_3rd.ly << EOF
\version "2.19.82"
...
cat > ./$workdir/${title}_Minor_sequences.ly << EOF
\version "2.19.82"
...
cat > ./$workdir/${title}_Minor_keys.ly << EOF
\version "2.19.82"
...
\pageBreak
\markup{"$title Minor Keys with four note slurred"}
\score {{
    \global
     \relative c' {
